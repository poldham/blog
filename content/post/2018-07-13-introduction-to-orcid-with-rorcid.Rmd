...
The employment records may be of different lengths. For example one oldham above has one entry and another has three. The `source_source_orcid_path` column is the key field foir identifying which oldham the records belong to.
...
We can see that these calls are becoming repetitive. We are calling an specific `rorcid` function and then extracting a specific field into a data frame, suggesting we could start thinking about a helper function. We won't go there now but a quick initial sketch for that might be. 
...
If you are new to list columns, or `purrr` in general, a great place to start is [Jenny Bryans purrr tutorial](https://jennybc.github.io/purrr-tutorial/ls13_list-columns.html). 
