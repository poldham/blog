---
title: Geocoding Scientific Literature with R
author: Paul Oldham
date: '2018-05-22'
slug: geocoding-scientific-literature-with-r
...
In this article we will look at geocoding using the Google Maps API and the `placement`, `ggmap`, and `googleway` packages in R. We will work with some raw data from Clarivate Analytics [`Web of Science`](http://wokinfo.com/) (formerly Thomson Reuters) database of scientific literature. Many universities have access to Web of Science and it is a very important tool in fields such as bibliometrics/scientometrics.  This article is part of work in progress for the WIPO Patent Analytics Handbook that addresses methods for advanced patent analytics including linking the scientific literature with patent literature. Comments and suggestions for improvements are welcome and can be provided by raising an issue on Github [here](https://github.com/poldham/blog/issues).
...
Let's take a look at the issues that can arise with the return from the Google Maps API. For the moment we will focus on the completeness of the data. 
...
write_csv(wos_abbreviations, "wos_abbreviations.csv")
```
...
```{r apply_reoplaceabbr, eval=FALSE}
...
This is not perfect, for example we encounter issues with Agriculture and Agricultural and so on. However, it is good enough for the time being. Rather than focus on resolving a small number of remaining items the next step is to reunite the fields we separated into a field we will call locations edited using the `tidyr` `unite` function. 

```{r create_locateionsedited, eval=FALSE}
lookup <- lookup %>%
...
- sign up for the API and obtain a key
- import dataset
- address case issues
