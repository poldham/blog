...
description: "Geocoding 5000 organisation names from Web of Science in R with the Google Maps API. Work in progress for the WIPO Patent Analytics Handbook."
categories:
  - R
tags:
  - bibliometrics
...
In this article we will look at geocoding using the Google Maps API and the `placement` and `ggmap` packages in R. We will work with some raw data from Clarivate Analytics `Web of Science` (formerly Thomson Reuters) database of scientific literature. Many universities have access to Web of Science and it is a very important tool in fields such as bibliometrics/scientometrics.  This article is part of work in progress for the WIPO Patent Analytics Handbook that addresses methods for advanced patent analytics including linking the scientific literature with patent literature. <!---But geocododed data is relatively new. There are important exceptions to this[10.1007/s11192-017-2463-2]. In the field of patent analytics geocoding is also relatively new with the USPTO patentsview service leading the way and the economists at WIPO publishing a REPORT that INCLUDED geocoding. ADD REFS AFTER LOOKUP--->

Geocoding is the process of taking a name and address and looking up the geographic coordinates expressed in latitude and longitude expressed in the form of coordinates. This is normally done using a web service. There are plenty of example walkthroughs on how to do this. However, many of them start with data that is either limited to city or country names or is already clean. We will be working with data that is really rather messy.   
...
By the end of this article you will be familiar with what geocoding is and how to carry out geocoding using the placement or ggmap packages in R with RStudio. You will also be familiar with the Google Maps API and be able to identify and retrieve missing data using packages from the tidyverse. 

<!---One of the big issues when working with the scientific literature or patent data is name cleaning. Normally I would do this before geocoding using match criteria in a tool such as [Vantage Point](https://www.thevantagepoint.com/). But in a future post I also want to explore whether obtaining the geocoded information, notably the street address and coordinates, can assist with name cleaning. That is, can geocoding help us to identify cases where organisations have very similar names and addresses? If so we can probably safely assume they are the same organisation. This remains to be seen. --->
...
Both the `placement` and the `ggmap` packages provide functions for geocoding with the Google Maps API. The `placement` package by [Derek Darves](http://derekyves.github.io/) was created in 2016 and provides straightgforward access to the Google Maps API and additional tools for address cleaning, calculating distances and driving times. As Derek explains [here](http://derekyves.github.io/2016/07/24/placement-pkg.html). I found it remarkably easy to use and it does not require any complicated code. The function we will be using is `geocode_url()` and `geocode_pull()`. That is basically it. 
...
We will mainly use the placement package because I like the simplicity of the package, but which you use will depend on your purpose and you will probably want to experiment with the wider functionality of ggmap. 
...
a) Save the text file somewhere sensible and copy it into the function below when needed. 
...
Below we will briefly show how to use the `placement`, `ggmap` and newer `googleway` packages to retrieve geocode data. Unfortunately the return from the Google API with placement also includes a column called `input_url`. I say unfortunate because the `input_url` includes your private API key! [CHEK AGAIN] So, if you are planning to make any of this data public you should exclude the `input_url` column. 
...
affiliation_records <- read_csv("~/Desktop/open_source_master/marine_wipo/google_geocoding/affiliation_records.csv") # replace with github url
```
...
library(placement)
coordaffil <- 
  geocode_url(affiliation_records$locations, auth = "standard_api", privkey = "yourkeygoeshere", clean = TRUE, add_date = 'today', verbose = TRUE)
```
...
When using `ggmap` I encountered a significant number of OVER_QUERY_LIMIT entries in the return. Why is something of a mystery although as discussed [here](https://stackoverflow.com/questions/36175529/getting-over-query-limit-after-one-request-with-geocode) this may because you are sharing the call to the free service with others. It is therefore better to get a key if you are going to be using this service for a while. To authenticate using ggmap (2.7 only) create a key based on the key in your environment. Pass it to `register_google()` and make the call. 
...
An alternative to ggmap is also available using the googleway package. As its name suggests this seems to focus more on directions than geocoding although that is fully supported. For example. 

```{r eval=FALSE}
library(googleway)
googleway <- google_geocode(address = "Flinders Street Station", key = key, simplify = TRUE)
```

As this makes clear, you have at least three choices for geocoding and which you prefer will depend on your needs. I found ggmap rather awkward because the existing CRAN version (2.6) does not provide the `register_google()` function in the long standing 2.7 development version. While this is a bit awkward `ggmap` provides some very poweful features that you will want to use.
...
Let's join the results onto the original table. Note that the return from Google includes an `input_url` column that contains the privatekey when using placement. This column has been dropped for this post. 
...
lookup %>% 
  ...
  select(records, locations, status) %>% 
  head() %>%
  knitr::kable()
```
...
save(wos_abbreviations, file = "wos_abbreviations.rda")
```
...
Next we want to iterate over the list of our original affiliation names and replace the abbreviations. There are a variety of ways to do that such as the `qdap` package function `multigsub` or `mgsub`. We like `qdap` a lot but installation of the package can be more than a bit awkward due to a dependency on `rJava`. If you would like to install `qdap` but run into problems with rjava on a Mac the instructions [here](https://github.com/MTFA/CohortEx/wiki/Run-rJava-with-RStudio-under-OSX-10.10,-10.11-(El-Capitan)-or-10.12-(Sierra)) can solve installation problems. Instead we are going to use a simple for loop.
...
How far we want to go with this will depend on our purpose at the time. For our purposes we would like to get as many as we reasonably can and certainly those with scores over 10 records as potentially important players in the scientific landscape for marine research in the South East Asia region. 
...
lookuplookup$name <- 
  stringr::str_replace_all(lookuplookup$name, "&", "and")
...
As note above, one significant problem is that some organisations may be located in more than one city (e.g. Chines National Academy of Science). To address this we start by searching with `name_city`. We then create a subset of that data where the results are *not found* to search with using `name_country`. That should hopefully avoid duplication. 
...
When we receive the results back we need to join the data back on to the original table. In the return from Google our original `name_city` column is called `locations` and so we start by renaming it. 

```{r name_city_join, eval=FALSE}
name_city <- name_city %>% 
  rename(., name_city = locations)

# drop unwanted columns
lookuplookup <- lookuplookup %>% 
  select(., records, locations, locations_edited, name_city, name_country, name)

# join the tables with bind_cols
...
```{r name_city_results}
name_city_results <- name_city_results %>%
  select(., -13:-14) # drop name_city1 and input_url
```
...
Next we filter the `name_city` results to create the `name_country` search and send that off to the Google API.
...
# send off the names
name_country_geo <- 
  placement::geocode_url(name_country$name_country, auth = "standard_api", privkey = "yourapikey", clean = TRUE, add_date = 'today', verbose = TRUE)
...
name_country <- 
  name_country %>% 
  select(., 1:6)
name_country_results <- 
  bind_cols(name_country, name_country_geo) %>%
  select(., -13:-14)
```
...
Let's start by pulling together the cases where results were found. We are going to create new objects that we are calling res1, res2, res3 to help us be clear about what we are combining together.
...
lookupresults %>%
  filter(., status == "OK") %>%
  count() # 249 found # why does this only have locations edited??? Address
names_city_country_results %>%
  filter(., status == "OK") %>%
  count() # 480 found
...
```{r bind_final}
res1 <- results %>%
  filter(., status == "OK") # 3947 found
res2 <- lookupresults %>%
  filter(., status == "OK") # 249 found
res3 <- names_city_country_results %>%
  filter(., status == "OK") # 480 found
results_found_final <- 
  bind_rows(res1, res2) %>%
  bind_rows(., res3) # 4676 total
```

Next we join the results table to our original affiliations table using left_join and locations as the id. 

```{r results_complete}
results_complete <- 
  left_join(affiliation_records, results_found_final, .id = "locations")

# check the number of rows
nrow(results_complete)
```

We will write the results to an Excel and csv file that we can use in other programmes 
